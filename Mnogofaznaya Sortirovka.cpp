...
Series MS(Series& SeriesOne, Series& SeriesTwo)
{
    Series NewSeries;
...
void SortUsingManyPhaseMerging(ifstream* file, int Lenght)
{
    ...
    *file >> SeriesArr[0].elements[0];
    ...
    while (AmountOfSeries > 1)
    {
        ...
        {
            Series NewSeries = MS(Buckets[IndexOfTheBiggerBucket].seriesArr[i], Buckets[IndexOfTheSmallerBucket].seriesArr[lastIndexOfSmallerBucket]);
